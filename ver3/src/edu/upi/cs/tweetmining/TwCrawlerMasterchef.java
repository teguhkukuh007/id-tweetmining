...
public class TwCrawlerMasterchef {
   ...
   public String query;
   public int tweetPerPage=20; //tweet per page, default 20
   
   private String nextQuery="";      //untuk page selanjutnya
   
   public TwCrawlerMasterchef() {
   ...
   public  void process() {        
        ...
        try {
            ...
            String strUrl = "http://search.twitter.com/search.json?"+q+"&rpp="+tweetPerPage+"&include_entities=true&result_type=recent"; 
            ...
            if (strOut.length()>0) {
               if (strOut.charAt(strOut.length()-1)!='}') {  //bukan JSON
             	  ...
             	  System.out.println("data yg salah="+strOut);
               } else {
    ...
    public static void main(String[] args) throws InterruptedException {
        //TBD: dikombinasikan dengan proses twmentah, jika duplikasi banyak, waktu diperlama, tapi jika duplikasi sedikit, waktu diperpendek
    	
    	//testing    	
    	TwCrawlerMasterchef tw = new TwCrawlerMasterchef();
    	tw.dbName = "localhost/masterchef2";
    	tw.userName = "yudi3";
    	tw.password = "rahasia";
    	tw.query="q=masterchefindonesia";
    	tw.tableName = "tw_mentah";
    	tw.tweetPerPage = 100;
    	
    	for (int i=0; i<=500 ; i++) {
            System.out.println("Proses masterchef ke---------------------------------->"+i);
            tw.process();
            //sleep menit
            if (isError) {
            	...
            	System.out.println("Selesai, tidur dulu 30 detik  menit, setelah menyelesaikan iterasi ke "+i);          
